...
int main()
{
	...
	w.Add("flecs.trigger.server", "export LD_LIBRARY_PATH=.:/usr/local/lib:/usr/local/lib64; cd /dev/shm/flecs-rpc/.build/server; ./flecs-server;");
	w.Add("flecs.trigger.client", "export LD_LIBRARY_PATH=.:/usr/local/lib:/usr/local/lib64; cd /dev/shm/flecs-rpc/.build/client; ./flecs-client;");
